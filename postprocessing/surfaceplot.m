function [pvis,tvis,u] = surfaceplot(mesh,elcon,UDG,ib,nref,clim)
...
if nref>0
  if size(tlocal,2)==3  
    A0=koornwinder(plocal(:,1:nd-1),porder);
    [plocal,tlocal]=uniref(plocal,tlocal,nref);
    A=koornwinder(plocal(:,1:2),porder)/A0;
  else
    A0=tensorproduct(plocal(:,1:nd-1),porder);
    m = porder*(nref+1)+1;
    [plocal,tlocal]=squaremesh(m,m,0,1);
    A=tensorproduct(plocal(:,1:nd-1),porder)/A0;  
  end
  npln=size(plocal,1);
