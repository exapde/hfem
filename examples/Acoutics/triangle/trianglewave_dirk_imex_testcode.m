...
% dt = 0.09;
dt = 0.001;
ntime = 5000; % explicit number of steps
...
%       figure(3); clf; scaplot(mesh,UDG(:,1,:),[-1 1],2,1); axis off; colormap jet;            
%      figure(4); clf; scaplot(mesh,PDG(:,1,:),[-1 1],2,1); axis off; colormap jet;            
...
%     figure(2); clf; scaplot(mesh,UDG(:,4,:),[-1 1],2,1); axis off; colormap jet;  
    ...
    % Implicit Code
    [UDGimp,UH,PDG] = hdg_wave_imex_coupled_test(master,mesh,app,UDG(:,1:3,:),UDG(:,4,:),time,dt,nstage,torder,Minv);   
%     [UDG,UH,PDG] = hdg_wave_imex_coupled(master,mesh,app,UDG,PDG,time,dt,nstage,torder, Minv);    

    UDG = cat(2,UDGimp,PDG); % Fixes notation differences between explicit and implicit codes
    
    figure(3); clf; scaplot(mesh,PDG(:,1,:),[-1 1],2,1); axis off; colormap jet; 
