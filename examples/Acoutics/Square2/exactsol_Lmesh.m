function uex = exactsol_Lmesh(p, time)

x = p(:,1,:);
y = p(:,2,:);

u=(1/(sqrt(2)*pi))*sin(2*pi*x).*sin(2*pi*y).*sin(sqrt(2)*pi*time);
v=sin(2*pi*x).*sin(2*pi*y).*cos(sqrt(2)*pi*time);
qx=(1/sqrt(2))*cos(2*pi*x).*sin(2*pi*y).*sin(sqrt(2)*pi*time);
qy=(1/sqrt(2))*sin(2*pi*x).*cos(2*pi*y).*sin(sqrt(2)*pi*time);
