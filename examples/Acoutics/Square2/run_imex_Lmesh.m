...
torder = 3;
hybrid = 'hdg';
nref = 2; % nref = 1 is no refinements

% dt_choose = [0.061, 0.01525/2, 0.00678, 0.003815]; % Will choose the appropriate regions for each porder
...
mesh = mkmesh_L_imex(porder);
mesh2 = mesh;
% Refine the Mesh
mesh = refinemesh(mesh,porder,nref);
% Determine if an element is explicit or implicit
% mesh = create_mesh_imex(mesh,dt_choose(porder)/nref,c,Kmax);
...
% % Create Fully Implicit and Explicit
% meshAll = mesh;
% meshAll.fimex = []; meshEXall = meshAll; meshIMall = meshAll; % TO BE SUPPRESSED !!!!
mesh.fimex = []; mesh2.fimex = []; meshEX = mesh; meshIM = mesh; % TO BE SUPPRESSED !!!!
meshIM.imexB = mesh.imexB;
meshEX.imexB = mesh.imexB;
...
% [  ~   ,meshIMall] = preprocess(master,meshIMall,hybrid);
% [master,meshEXall] = preprocess(master,meshEXall,hybrid);
% [master,meshAll]   = preprocess(master,meshAll,hybrid);


%%%%%%%%%%%% MESH FOR POSTPROCESSING %%%%%%%%%%%%%%%%%
% Creation of the Mesh and Preprocessing
mesh1 = mkmesh_L_imex(porder+1);
% Refine the Mesh
...
% Create Fully Implicit and Explicit
meshAll1 = mesh1;
meshAll1.fimex = []; meshEXall1 = meshAll1; meshIMall1 = meshAll1; % TO BE SUPPRESSED !!!!
...
% [  ~   ,meshIMall1] = preprocess(master1,meshIMall1,hybrid);
% [master1,meshEXall1] = preprocess(master1,meshEXall1,hybrid);
% [master1,meshAll1]   = preprocess(master1,meshAll1,hybrid);


%%%%%%%%%%%%%% INITIALIZATION %%%%%%%%%%%%%%%%
% L2-projection of the initial sol on implicit mesh
SOLimp = l2eprojection(meshIM,master,@exactsol_Lmesh,[],0,4);
UDGimp(:,:,:) = SOLimp(:,1:3,:);
PDGimp(:,:,:) = SOLimp(:,4,:);
...
SOLexp = l2eprojection(meshEX,master,@exactsol_Lmesh,[],0,4);
UDGexp(:,:,:) = SOLexp(:,1:3,:);
PDGexp(:,:,:) = SOLexp(:,4,:);
...
% % Initiate Purely Implicit and Explicit Codes
% SOLimp = l2eprojection(meshIMall,master,@exactsol_Lmesh,[],0,4);
% UDGimpAll(:,:,:) = SOLimp(:,1:3,:);
% PDGimpAll(:,:,:) = SOLimp(:,4,:);
% UHimpAll = inituhat(master,meshIMall.elcon,UDGimpAll,app.ncu);
% 
% 
% SOLexp = l2eprojection(meshEXall,master,@exactsol_Lmesh,[],0,4);
% UDGexpAll = SOLexp(:,1:3,:);
% PDGexpAll = SOLimp(:,4,:);
% UHexpAll = inituhat(master,meshEXall.elcon,UDGexpAll,app.ncu);
% 
% [MinvAll]=massinv(master, meshEXall);
...
for itime = 1:ntime
    ...
    % IMEX SOLVER
    [UDGexp,UDGimp,UHexp,UHimp,PDGexp,PDGimp] = hdg_solve_IMEX_Exp_Only(master,meshEX,meshIM,app,UDGexp,UDGimp,UHexp,UHimp,PDGexp,PDGimp,Minv,time,dt,nstage);
    
%     [UDGexpAll,UDGimpAll,UHexpAll,UHimpAll,PDGexpAll,PDGimpAll] = hdg_solve_IMEX(master,meshEXall,meshIMall,app,UDGexpAll,UDGimpAll,UHexpAll,UHimpAll,PDGexpAll,PDGimpAll,MinvAll,time,dt,nstage);
...
% IMEXdata = getVhatIMEX(meshIMall,UHimpAll);
% [UHexpAll,QHexpAll] = mkfhat(meshEXall,app,UDGexpAll);
...
UDGexp = cat(2, UDGexp, PDGexp);
UDGimp = cat(2, UDGimp, PDGimp);

% UDGexpAll = cat(2, UDGexpAll, PDGexpAll);
% UDGimpAll = cat(2, UDGimpAll, PDGimpAll);

% erruexp = calerror_IM_reference(UDGexp,meshEX,master,UDGimpAll,meshAll,itime*dt);
% erruimp = calerror_IM_reference(UDGimp,meshIM,master,UDGimpAll,meshAll,itime*dt);
...
figure(1); clf; scaplot(meshEX,UDGexp(:,1,:),[],2,1); axis off;
% figure(3); clf; scaplot(meshEX,abs(uexEX(:,1,:)-UDGexp(:,1,:)),[],2,1); axis off;
figure(2); clf; scaplot(meshIM,UDGimp(:,1,:),[],2,1); axis off;
% figure(5); clf; scaplot(meshIM,abs(UDGimp(:,1,:)-uexIM(:,1,:)),[],2,1); axis off;
figure(3); clf; scaplot(meshIMall,UDGimpAll(:,1,:),[],2,1); axis off;
figure(4); clf; scaplot(meshEXall,UDGexpAll(:,1,:),[],2,1); axis off;
figure(5); clf; scaplot(meshEX,UDGexp(:,1,:),[],2,1); axis off;
scaplot(meshIM,UDGimp(:,1,:),[],2,1); axis off;
