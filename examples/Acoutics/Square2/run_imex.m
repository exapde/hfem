...
% dt_choose = [0.13/2, 0.03/2, 0.0125/2, 0.007]; % Will choose the appropriate regions for each porder
...
mesh = create_mesh_imex(mesh,dt_choose,c,Kmax); % for time convergence
...
mesh.fimex = []; mesh2.fimex = []; meshEX = mesh; meshIM = mesh2; % TO BE SUPPRESSED !!!!
meshIM.imexB = mesh.imexB;
meshEX.imexB = mesh.imexB;
% Preprocess all the meshes
% [  ~   ,meshIM] = preprocess(master,meshIM,hybrid);
[  ~   ,meshIM] = preprocess(master2,meshIM,hybrid);
