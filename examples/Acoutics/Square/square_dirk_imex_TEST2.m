...
for itime = 1:ntime
    ...
    % Implicit Code
%     UDGimex = hdg_vibration_imex_coupled_UPDATED(master,mesh,app,UDG(:,1:3,:),UDG(:,4,:),time,dt,nstage,torder,Minv); 
...
%     UDGex = ehdgrk_imex_coupled(master,mesh,app,Minv,UDG);
%     UDGex2 = ehdgrk(master,mesh,app,Minv,UDG);
...
erru = calerror(UDGimex,mesh,master,@exactsol,[],itime*dt,[1 2 3 4]);
% erruex = calerror(UDGex,mesh,master,@exactsol,[],itime*dt,[1 2 3 4]);
...
% erruimp(:,i,j) = calerror(UDGimp,mesh,master,@exactsol,[],itime*dt_loop,[1 2 3 4]);
% erru2 = calerror(UDG2,mesh,master,@exactsol,[],itime*dt,[1 2 3 4]);

uex = exactsol(mesh.dgnodes, [], itime*dt);
figure(1); clf; scaplot(mesh,uex(:,4,:),[],2,1); axis off;
figure(2); clf; scaplot(mesh,UDGimex(:,4,:),[],2,1); axis off;
