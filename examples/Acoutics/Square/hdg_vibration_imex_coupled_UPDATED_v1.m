function UDG = hdg_vibration_imex_coupled_UPDATED_v1(master,mesh,app,VDGn,Un,tn,dt,nstage,torder,Minv)
...
%     [UDG,B,G] = ehdgrk_imex_Ru_newform(master,mesh,app,Minv,cat(2,UDG,Uni));
    % check UDG 
    [UHAT,B,G] = ehdgrk_imex_coupled_BG(master,mesh,app,Minv,UDGo, i,3);
...
%         imp_exp = mesh.imex(E_neighbors(count,:)); % determines if neighboring element is implicit or explicit
    ...
    for j = 1:3 % loop over each face of the element i
        ...
        I = perm(:,j,1);  % Changed from 2 to 1, was no 2nd perm
        J = ((j-1)*nps+1):j*nps;
