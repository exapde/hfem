...
%         SDG = SDG - (D(istage,jstage)/dt)*(UDGall(:,1:nc,ind,jstage)-VDGn(:,:,ind));
    ...
    % compute (v,q,vhat)
%     [VDGni,VHni] = hdg_solve(master,mesh,app,SDG,fc,tni, [VDGni, Uni], Minv, Uni);
...
% check UDG 
    [UHAT,B,G,D] = ehdgrk_imex_coupled_UPDATED(master,mesh,app,Minv,UDG, i,istage);
