function UDG = hdg_vibration_imex_coupled_UPDATED_v5(master,mesh,app,VDGn,Un,tn,dt,nstage,torder,Minv)
...
% Find explicit elements
Eind = find(mesh.imex == 0); % Explicit element indicies


% Finds imex boundary explicit elements
E_ind = find(mesh.bound_ele(:,1) > 0); % Imex boundary faces
E_ele = mesh.bound_ele(E_ind,1); % Imex boundary explicit elements
expE = length(E_ind); % number of explicit elements


% ind = sort(cat(1,ind,E_ele));
ind = cat(1,ind,E_ele); % indicies of implicit elements and boundary explicit elements
...
UDGall = zeros(npv,nc+ncu,ne,nstage+1); % changing to nstage+1 for explicit
UDGall(:,:,:,1) = cat(2, VDGn, Un); % add on column for explicit calc
UDG = cat(2, VDGn, Un);
UDGo = zeros(npv,nc+ncu,ne,nstage+1);
UDGo(:,:,:,1) = UDG;
% UDGni = UDG;
...
% Define time
 ...
 SDG = zeros(npv,nc,ne);
 SH = zeros(npv,1,ne);
 UDGni = zeros(npv,nc+1,ne,nstage+2);
 UDGni(:,:,:,1) = UDG;
 
 
 % First explicit solve
 [UDGni(:,:,:,2),bhat,Dhat] = ehdgrk_imex_coupled_v3(master,mesh,app,Minv,UDGni,1);
%  UDGall(:,:,Eind,2);
...
for istage = 1:nstage % for each dirk stage    
    ...
    % compute (v,q,vhat)
%     [VDGni,VHni] = hdg_solve(master,mesh,app,SDG,fc,tni, VDGni, Minv, Uni,dt,istage,UDGo);
    [VDGni,VHni] = hdg_solve(master,mesh,app,SDG,fc,tni, VDGn, Minv, Un,dt,istage,UDGni);
    ...
    UDGall(:,1:nc,Eind,istage) = UDGni(:,1:nc,Eind,istage+1);

    % update the source term for v = du/dt  
    SH(:,1,ind) = UDGall(:,1,ind,istage) + fc*Un(:,:,ind);
    for jstage = 1:istage-1
        SH(:,1,ind) = SH(:,1,ind) - (D(istage,jstage)/dt)*(UDGall(:,end,ind,jstage)-Un(:,:,ind));
    end        
    
    
    % update intermediate states for u
    Uni = (dt/D(istage,istage))*SH;
    UDGall(:,end,:,istage) = Uni;

    % Explicit Code
    UDGni(:,:,:,istage+2) = ehdgrk_imex_coupled_v3(master,mesh,app,Minv,UDGni,istage+1);
...
for i=1:nstage+1
   for j=1:nstage+1     
      VDGnew(:,:,Eind)=VDGnew(:,:,Eind)+bhat(i)*Dhat(i,j).*((UDGni(:,1:nc,Eind,j+1)-VDGn(:,:,Eind)));
      Unew(:,:,Eind)=Unew(:,:,Eind)+bhat(i)*Dhat(i,j).*((UDGni(:,end,Eind,j+1)-Un(:,:,Eind)));
   end
end
...
function [VDG,VH] = hdg_solve(master, mesh, app, SDG, fc, time, UDG, Minv, Uni,dt,istage,UDGo)
%
...
%         elseif mesh.fimex(abs(mesh.t2f(i,j)))== 1
%         
        else
            % form N_K
...
%         end
    ...
    H(:,:,count) = H_K;  %changed to i from count
    R(:,count)   = R_K; 
end
...
% check UDG 
    [UHAT,B,G,D,UDG_stage] = ehdgrk_imex_coupled_BG_v4(master,mesh,app,Minv,UDGo, i,istage);
    
    UHAT = reshape(UHAT,nps,3,nt);
%     [UDG] = ehdgrk_imex_coupled_v2(master,mesh,app,Minv,UDGo,4); 
    
    %     [UHAT,B,G,D] = ehdgrk_imex_coupled_UPDATED(master,mesh,app,Minv,UDG, i,3);
...
%         imp_exp = mesh.imex(E_neighbors(count,:)); % determines if neighboring element is implicit or explicit
    ...
    for j = 1:3 % loop over each face of the element i
        ...
        % Find faces bounding explicit elements
        if en ~= 0 && mesh.fimex(abs(mesh.t2f(i,j)))~= 1   % Boundary is an explicit element
%             face = find(mesh.t2t(en,:) == i);
%             Ineigh = perm(:,face,1);
%             Ineigh = fliplr(Ineigh')'; % flips order of points
%             gn = UHAT(:,j,en);
            gn = UHAT(:,j,i);
...
%     P(:,:,count2) = -Minv_Blk*B*dtj;
%     L(:,count2) = Minv_Blk*G*dtj;
...
% total number of elements being computed here
impE = impE + expE;
ind = sort(cat(1,ind,E_ele));
% ind = cat(1,ind,E_ele);
...
[elcon,ndof,edg,hybridn] = elconnectivities_imex_UPDATED(mesh,nf,E_ele);  % changed from E_ele to ind
