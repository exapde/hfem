function UDG = hdg_vibration_imex_coupled_UPDATED_v3(master,mesh,app,VDGn,Un,tn,dt,nstage,torder,Minv)
...
%     
    % Explicit Code
    UDGni = ehdgrk_imex_coupled_v2(master,mesh,app,Minv,UDGall(:,:,:,:),istage);
...
%     UDGo = UDGall(:,:,:,istage);  % putting in to calc new original val at timestep
...
[UDG] = ehdgrk_imex_coupled_v2(master,mesh,app,Minv,UDGall,4);
% [UHAT,B,G,D] = ehdgrk_imex_coupled_UPDATED_v2(master,mesh,app,Minv,UDGall, i,istage);
...
%         end
        ...
        if mesh.fimex(abs(mesh.t2f(i,j)))== 1
            ...
            ug = (sh1d(:,:,1))'*(UDG(I,:,en)); % solution on the explicit element
            gn = ug(:,2).*nl(:,1) + ug(:,3).*nl(:,2) + tau*ug(:,1); % q_E dot n + tau_v_E
            % gn = UHAT from the explicit code. UHAT can be stored and used
            % here. 
            G_K(J,1) = G_K(J,1) + sh1d(:,:,1)*(dws.*gn);
        end  
    end
