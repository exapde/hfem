...
% % % %             for k = 1:nfe        
% % % %                 for m = 1:npf   
% % % %                     localNode = find(meshp{i}.elcon(m,k,j) == meshp{i}.entpart);
% % % %                     meshp{i}.elcon(m,k,j) = localNode;
% % % %                     if length(localNode) ~= 1 ; error('Error No. 1 in domaindecomposition2'); end
% % % %                 end
% % % %             end
% % % %         end
        ...
        for j = 1:ne
            for k = 1:size(mesh.t2f,2)
                if mesh.t2f(meshp{i}.elempart(j),k) > 0
                    meshp{i}.t2f(j,k) = faceMapping(mesh.t2f(meshp{i}.elempart(j),k));
                else
                    meshp{i}.t2f(j,k) = mesh.t2f(meshp{i}.elempart(j),k);
                end
            end
        end
        
%         faceMapping = zeros(max(facesInProcessor)+1,1);
%         faceMapping(facesInProcessor) = 1:length(facesInProcessor);
%         faceMapping(facesInProcessor+1) = length(facesInProcessor)+1;
% 
%         meshp{i}.t2f = faceMapping(mesh.t2f(meshp{i}.elempart(:),:));
%         [j_aux, k_aux] = find(meshp{i}.t2f) == length(facesInProcessor)+1;
%         meshp{i}.t2f(j_aux, k_aux) = mesh.t2f(meshp{i}.elempart(j_aux),k_aux);
            
% % % %         for j = 1:ne
% % % %             for k = 1:size(mesh.t2f,2)
% % % %                 if mesh.t2f(meshp{i}.elempart(j),k) > 0
% % % %                     localFace = find(mesh.t2f(meshp{i}.elempart(j),k) == facesInProcessor);
% % % %                     meshp{i}.t2f(j,k) = localFace;
% % % %                     if length(localFace) ~= 1 ; error('Error No. 2 in domaindecomposition2'); end
% % % %                 else
% % % %                     meshp{i}.t2f(j,k) = mesh.t2f(meshp{i}.elempart(j),k);
% % % %                 end
% % % %             end
...
%         [~,j1] = find(mesh.elcon==indent{i}(j));
        [~,j1] = find(mesh.elcon(:,indelem{i})==indent{i}(j));
        j1 = indelem{i}(j1);
        if isempty(j1); error('No element in processor neighbors one EDG node in the processor'); end
        faces = mesh.t2f(j1,:);
        faces = unique(faces(:));
        potentialElements = mesh.f(faces,end-1:end);        % potential elements to be neighboring the node
        ...
        while true
            jj = jj+1;
            if ~isempty(find(mesh.elcon(:,totalPotentialElements(jj))==indent{i}(j),1))
                j1 = [j1;totalPotentialElements(jj)];
                faces = mesh.t2f(totalPotentialElements(jj),:);
                faces = unique(faces(:));
                morePotentialElements = mesh.f(faces,end-1:end);
