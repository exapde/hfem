...
nve = size(mesh.t,2);
permMesh = mesh.perm;
permgeomMesh = mesh.permgeom;
neMesh = mesh.ne;
hybrid = mesh.hybrid;
blkSizeMesh = mesh.blkSize;
ndMesh = mesh.nd;
tlocfcMesh = mesh.tlocfc;

elcon_tmp = cell(nproc,1);
dgnodes_tmp = cell(nproc,1);
bf_tmp = cell(nproc,1);
t2f_tmp = cell(nproc,1);
t_tmp = cell(nproc,1);
p_tmp = cell(nproc,1);
elcon_reshaped = reshape(mesh.elcon,[npf nfe neMesh]);
for i=1:nproc
    elcon_tmp{i} = elcon_reshaped(:,:,meshp{i}.elempart);
    dgnodes_tmp{i} = mesh.dgnodes(:,:,meshp{i}.elempart);
    bf_tmp{i} = mesh.bf(:,meshp{i}.elempart);
    t2f_tmp{i} = mesh.t2f(meshp{i}.elempart,:);
    t_tmp{i} = mesh.t(meshp{i}.elempart,:);
    p_tmp{i} = mesh.p(unique(t_tmp{i}(:)),:);
end
clear elcon_reshaped

parfor i=1:nproc % loop over each subdomain
    disp(['Processing subdomain No. ',num2str(i)]);
    meshp{i}.perm = permMesh;
    meshp{i}.permgeom = permgeomMesh;
    meshp{i}.dgnodes = dgnodes_tmp{i}; %dgnodesMesh(:,:,meshp{i}.elempart);
    meshp{i}.bf = bf_tmp{i}; % bfMesh(:,meshp{i}.elempart);
    
    % compute local element-to-entity connectivities
%     elcon = reshape(elconMesh,[npf nfe neMesh]);
    meshp{i}.elcon = elcon_tmp{i}; % elcon(:,:,meshp{i}.elempart);
    ne = length(meshp{i}.elempart);        
    if strcmp(hybrid,'hdg')
        t2f = t2f_tmp{i}; % t2fMesh(meshp{i}.elempart,:);
        for j = 1:ne
            for k = 1:nfe
        ...
        for j = 1:ne
            for k = 1:nfe
                ii = elcon_tmp{i}(:,k,j) - (t2f_tmp{i}(j,k)-1)*npf;                
    ...
    elseif strcmp(hybrid,'edg') || strcmp(hybrid,'iedg')
        ...
        facesInProcessor = unique(t2f_tmp{i}(:));
        ...
        meshp{i}.t2f = faceMapping(t2f_tmp{i});
    end
    meshp{i}.elcon = reshape(meshp{i}.elcon,[npf*nfe ne]);   
    meshp{i} = block_crs(meshp{i},hybrid);
    ...
    if numRealEntities ~= numTotalEntities
        error('There are ghost (missing) entities in the mesh.');       % The construction of BJ and BK from cbsr is based on the assumption that there are no ghost entities
    end
    ...
    meshp{i}.nve = nve;
    meshp{i}.nvf = size(tlocfcMesh,2);
    meshp{i}.nv  = size(p_tmp{i},1);
    if strcmp(hybrid,'hdg')
...
parfor i=1:nproc
    ...
    j1ByNode = cell(length(indent{i}),1);
    for j = 1:length(indent{i})
%         [~,j1] = find(mesh.elcon==indent{i}(j));
        [~,j1] = find(elconMesh(:,indelem{i})==indent{i}(j));
        j1 = indelem{i}(j1);
        if isempty(j1); error('No element in processor neighbors one EDG node in the processor'); end
        faces = t2fMesh(j1,:);
        faces = unique(faces(:));
        potentialElements = fMesh(faces,end-1:end);        % potential elements to be neighboring the node
        potentialElements = reshape(potentialElements(potentialElements > 0),[],1);
        potentialElements = unique(potentialElements);
        potentialElements = setdiff(potentialElements(:),j1(:));
        totalPotentialElements = potentialElements;
        jj = 0;
        while true
            jj = jj+1;
            if ~isempty(find(elconMesh(:,totalPotentialElements(jj))==indent{i}(j),1))
                j1 = [j1;totalPotentialElements(jj)];
                faces = t2fMesh(totalPotentialElements(jj),:);
                faces = unique(faces(:));
                morePotentialElements = fMesh(faces,end-1:end);
                morePotentialElements = reshape(morePotentialElements(morePotentialElements > 0),[],1);
                morePotentialElements = unique(morePotentialElements);
                morePotentialElements = setdiff(morePotentialElements,[totalPotentialElements;j1]);
                totalPotentialElements = [totalPotentialElements; morePotentialElements];
            end
            if length(totalPotentialElements) == jj; break; end
        end
        j1 = unique(j1);
        edg1 = elconMesh(:,j1);
    ...
    indexInterfaceEnt = find(edgnumber==1);
    intf = indent{i}(indexInterfaceEnt);
...
%         j1 = unique(j1);
        j1 = j1ByNode{indexInterfaceEnt(j)};
        edg1 = elconMesh(:,j1);
        edg1 = unique(edg1(:));
...
%         j1 = unique(j1);
    ...
    for j = 1:length(elem1)        
        if all(ismember(elconMesh(:,elem1(j)),indent{i}))            
            in(j) = 1;
        end
    end  
    elem1 = elem1(in==0);    
