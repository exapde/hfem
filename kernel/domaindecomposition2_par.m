...
function [mesh,meshp,dmd] = domaindecomposition2_par(mesh,nproc,DDobjectiveFlag,ent2entWeight)
...
mesh_perm = mesh.perm;
mesh_permgeom = mesh.permgeom;
mesh_dgnodes = mesh.dgnodes;
mesh_bf = mesh.bf;
mesh_elcon = mesh.elcon;
mesh_ne = mesh.ne;
hybrid = mesh.hybrid;
mesh_t2f = mesh.t2f;
mesh_t = mesh_t;
...
parfor i=1:nproc % loop over each subdomain
    ...
    meshp_perm{i} = mesh_perm;
    meshp{i}.permgeom = mesh_permgeom;
    meshp{i}.dgnodes = mesh_dgnodes(:,:,meshp{i}.elempart);
    meshp{i}.bf = mesh_bf(:,meshp{i}.elempart);
    
    % Compute local element-to-entity connectivities
    elcon = reshape(mesh_elcon,[npf nfe mesh_ne]);
    meshp{i}.elcon = elcon(:,:,meshp{i}.elempart);
    ne = length(meshp{i}.elempart);
    if strcmp(hybrid,'hdg')
        t2f = mesh_t2f(meshp{i}.elempart,:);
        for j = 1:ne
            for k = 1:nfe
        ...
        for j = 1:ne
            for k = 1:nfe
                ii = elcon(:,k,meshp{i}.elempart(j)) - (mesh_t2f(meshp{i}.elempart(j),k)-1)*npf;                
    ...
    elseif strcmp(hybrid,'edg') || strcmp(hybrid,'iedg')
        ...
        facesInProcessor = mesh_t2f(meshp{i}.elempart,:);
        ...
        for j = 1:ne
            for k = 1:nfe
                if mesh_t2f(meshp{i}.elempart(j),k) > 0
                    meshp{i}.t2f(j,k) = faceMapping(mesh_t2f(meshp{i}.elempart(j),k));
                else
...
%     end    
    ...
    meshp{i}.nve = size(mesh_t,2);
    meshp{i}.nvf = size(tlocfc,2);
    t = mesh_t(meshp{i}.elempart,:);
    p = mesh_p(unique(t(:)),:);
...
parfor i=1:nproc
    ...
    for j=1:length(indelem{i})
        if length(setdiff(indent{i},mesh_t2f(indelem{i}(j),:))) == length(indent{i})
%             entitiesInElement = mesh_t2f(indelem{i}(j),:);
...
%             warning('An element in the processor had did not have any entity in the processors. The issue has been fixed successfully.');
            error('An element in the processor does not have any entity in the processors. The issue cannot be solved with parfor.');
        end
    end
...
% loop over each subdomain
mesh_cbsr_rowent2elem = mesh.cbsr_rowent2elem;
mesh_cbsr_colent2elem = mesh.cbsr_colent2elem;
mesh_f2f = mesh.f2f;
mesh_f = mesh.f;
parfor i=1:nproc    
    disp(['Proc. No. ', num2str(i),'/',num2str(nproc)]);
    ...
    for j = 1:length(indent{i})
        nj = indent{i}(j);
        rj = (mesh_cbsr_rowent2elem(nj)+1):mesh_cbsr_rowent2elem(nj+1);
        newElem = mesh_cbsr_colent2elem(rj);
...
parfor i=1:nproc
    ...
    elementsToRemove = [];
    for j=1:length(indelem{i})
        if length(setdiff(indent{i},mesh_elcon(:,indelem{i}(j)))) == length(indent{i})
%             entitiesInElement = mesh.elcon(:,indelem{i}(j));
%             candidateProcessors = entpart(entitiesInElement);
%             k = mode(candidateProcessors);
%             indelem{k} = sort([indelem{k};indelem{i}(j)]);
%             elementsToRemove = [elementsToRemove;indelem{i}(j)];
%             warning('An element in the processor had did not have any entity in the processors. The issue has been fixed successfully.');
            error('An element in the processor does not have any entity in the processors. The issue cannot be fixed with parfor.');
        end
    end
    indelem{i} = setdiff(indelem{i},elementsToRemove);
end
...
mesh_cbsr_rowent2elem = mesh.cbsr_rowent2elem;
mesh_cbsr_colent2elem = mesh.cbsr_colent2elem;
mesh_cbsr_rowpts = mesh.cbsr_rowpts;
mesh_cbsr_colind = mesh.cbsr_colind;
parfor i=1:nproc    
    ...
    for j = 1:nent
        nj = indent{i}(j);
        rj = (mesh_cbsr_rowpts(nj)+1):mesh_cbsr_rowpts(nj+1);
        edg1 = mesh_cbsr_colind(rj); % list of neirghboring edg nodes
        if all(ismember(edg1,indent{i}))
            % edgnode indent{i}(j) is fully inside the subdomain i (all
            % neighboring edg nodes are in the subdomain i)
            edgnumber(j) = 2;
...
%   find all elements that are connected to interface edgnodes
    ...
    for j = 1:length(intf)        
        nj = intf(j);
        rj = (mesh_cbsr_rowent2elem(nj)+1):mesh_cbsr_rowent2elem(nj+1);
        j1 = mesh_cbsr_colent2elem(rj); % list of neirghboring elements                
        in = ismember(j1,indelem{i});
        elem0 = [elem0; j1(in==0)];
        elem1 = [elem1; j1(in==1)];
    end
