...
function [mesh,meshp,dmd] = domaindecomposition2a(mesh,nproc,DDobjectiveFlag,ent2entWeight)
...
%     er=meshp{i}.BJ_rowpts-meshp{i}.BJ_rowpts2;max(abs(er(:)))
%     er=meshp{i}.BJ_colind-meshp{i}.BJ_colind2;max(abs(er(:)))
%     er=meshp{i}.BK_rowpts-meshp{i}.BK_rowpts2;max(abs(er(:)))
%     er=meshp{i}.BK_colind-meshp{i}.BK_colind2;max(abs(er(:)))
%     er=meshp{i}.BK_rowind-meshp{i}.BK_rowind2;max(abs(er(:)))
...
%                     % elements contain both edgjg and edgkg
%                     jke = je(ismemberBuiltinTypes(je,ke));                    
%                     % if any one of jke is not in the subdomain i
...
%             nedgkg = meshp{ncpu}.entpart(nedgk);
%             
%             [~,b1]=ismember(edgkg,nedgkg);
%             if max(abs(edgkg-nedgkg(b1)))>0
%                 error('something wrong');                        
%             end
%             meshp{i}.matrecv = [meshp{i}.matrecv; ncpu*ones(length(edgkg),1) jr'-1];                                                                                                                            
%             meshp{ncpu}.matsend = [meshp{ncpu}.matsend; i*ones(length(edgkg),1) njr(b1)'-1];             
%             
% %             for k = 1:length(edgkg)
% %                 meshp{i}.matrecv = [meshp{i}.matrecv; ncpu jr(k)-1];                                                                                                                            
% %                 nk = find(nedgkg==edgkg(k));
