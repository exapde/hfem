function [mesh,meshp,dmd] = domaindecomposition(mesh,nproc,DDobjectiveFlag,ent2entWeight)
...
for i=1:nproc % loop over each subdomain
    ...
    meshp{i}.BK_rowpts = zeros(nownent+1,1);
    meshp{i}.BK_colind = 0*meshp{i}.cbsr_colind;
    meshp{i}.BK_rowind = zeros(nownent+1,1);
    n = 1;
    m = 1;
    k = 1;
    for j = 1:nownent
        ...
        if ~isempty(c2)
            lc2 = length(c2);
            meshp{i}.BK_rowpts(m+1) = meshp{i}.BK_rowpts(m)+lc2;            
            meshp{i}.BK_colind(k:1:(k+lc2-1)) = c2;
    ...
    meshp{i}.BK_colind(meshp{i}.BK_colind==0)=[];  
    meshp{i}.BK_rowind(meshp{i}.BK_rowind==0)=[];   
    meshp{i}.BK_rowpts(meshp{i}.BK_rowpts==0)=[];       
    meshp{i}.BK_rowpts = [0; meshp{i}.BK_rowpts];
