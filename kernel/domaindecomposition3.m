...
function [mesh,meshp,dmd] = domaindecomposition3(mesh,nproc,DDobjectiveFlag,ent2entWeight)
% Domain decomposition for 2 element-overlap preconditioner
...
for i=1:nproc    
    ...
    elem2 = mesh.f(f0bis,end-1:end);
    i1 = find(elem2(:)>0);
    in = ones(size(elem2));
    in(i1) = ismember(elem2(i1),indelem{i});
    elem2 = unique(elem2(in==0));
    elem0 = unique([elem0; elem2]);
...
error('2-level overlap for EDG has not been validated yet.');
