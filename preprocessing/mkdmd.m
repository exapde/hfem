...
[all_elem2cpu,all_extintelem,all_extintelempts,all_face2cpu,all_extintface,all_extintfacepts,all_ent2cpu,...
 all_extintent,all_extintentpts] = overlappingpartition(t2f,t,elcon,facecon,hybrid,overlappinglevel,nproc,1);
...
%     extelem = setdiff((1:size(elcon,2))',intelem);
...
%         extintent = t2f(:,extintelem);
%         extintent = unique(extintent(:));
%         extent = setdiff(extintent,intent);
%         extintent = [intent; extent];
%         extintentpts = [length(intent) length(extent)];
%         ent2cpu = all_face2cpu(extintent)+1;  
    else
    ...
    for j = 1:nent  % For each entity in in intent
        ...
        entnb = colent2ent(rj); % list of its neighboring entities 
    ...
    % list of all interface eentities in the subdomain
    ...
    % find all neighboring entities that are connected to interface entities
    nbent0 = [];
    for j = 1:length(intfent)
        nj = intfent(j);
        ij = ent2ind(nj);
        rj = (rowent2ent(ij)+1):rowent2ent(ij+1);
        nbent1 = colent2ent(rj); % list of neighboring entities
        nbent1 = unique(nbent1(:));
        in = ismember(nbent1,intent);
        nbent0 = [nbent0; nbent1(in==0)];                
    end
    nbent0 = unique(nbent0); % list of all entities that are not in the subdomain but are connected to entities in the subdomain
    extent = nbent0;
    if isempty(setdiff(extent,extintent))==0
        setdiff(extent,extintent)        
        pause
    end
    
    %   find all elements that are connected to interface entities
    ...
    for j = 1:length(intfent)        
        ...
        ij = ent2ind(nj);
        rj = (rowent2elem(ij)+1):rowent2elem(ij+1);                
        j1 = colent2elem(rj); % list of neighboring elements                
        in = ismember(j1,intelem);
        elem0 = [elem0; j1(in==0)];
        elem1 = [elem1; j1(in==1)];
    end
    elem0 = unique(elem0);
    ...
    if ~all(ismember(elem2,intelem)); error('Something wrong.'); end
    extelem = elem0;
    intfelem = [elem2; elem1];
    ...
    [~,dmd{i}.elemmap] = ismember(dmd{i}.elempart,extintelem);
    ...
    [~,ond] = ismember(extelem,extintelem); dmd{i}.elemrecv(:,1) = elem2cpu(ond);
    ...
    [~,dmd{i}.entmap] = ismember(dmd{i}.entpart,extintent);
    ...
    [~,ond] = ismember(extent,extintent); dmd{i}.entrecv(:,1) = ent2cpu(ond);
