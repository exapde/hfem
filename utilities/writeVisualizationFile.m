...
function writeVisualizationFile(fileName, mesh, UDG, solType)
...
% NOTE:
% - If UH is to be visualized in the future, be careful with periodic boundaries

% if mesh.nd ~= 3; error('writeVisualizationFile only implemented for 3D simulations.'); end
if nargin < 4; solType = 0; end
...
elseif solType == 1
    gam  = 1.4;
    r    = UCG_p1(:,1);
    uv   = UCG_p1(:,2)./UCG_p1(:,1);
    vv   = UCG_p1(:,3)./UCG_p1(:,1);
    wv   = UCG_p1(:,4)./UCG_p1(:,1);
    E    = UCG_p1(:,5)./UCG_p1(:,1);
    p    = (gam-1)*r.*(E-0.5*(uv.^2+vv.^2+wv.^2));
    vMag = sqrt((uv.^2+vv.^2+wv.^2));
    rx   = UCG_p1(:,ncu+1);
    rvx  = UCG_p1(:,ncu+3); 
    ry   = UCG_p1(:,2*ncu+1);
    ruy  = UCG_p1(:,2*ncu+2);
    ...
    uy   = (ruy - ry.*uv).*r1;
    vort_z = vx-uy;
    mach = sqrt((uv.^2+vv.^2+wv.^2)./(gam*(gam-1)*(E-0.5*(uv.^2+vv.^2+wv.^2))));
    mach = real(mach);
    
    solutionMatrix = [UCG_p1, uv, wv, vMag, p, vort_z, mach];
end
