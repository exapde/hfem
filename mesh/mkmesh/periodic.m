...
% % % %mesh.t(ebnd2(j),localNodes(lf2)) = mesh.t(ebnd1(j),localNodes(lf1))
% % % newt = zeros(size(mesh.t));
% % % for e1=1:mesh.ne
% % %     newt(i,:) = mesh.t(i,:);
% % % 
% % %     for j=1:nfe
% % %         face = mesh.t2f(e1,j);
% % %         e2 = mesh.t2t(e1,j);
% % %         ps1 = mesh.t(e1,fn2en(j));
% % %         if e2 >= 0 && e2 < e1
% % %             lf2 = find(face == mesh.t2f(e2,:));
% % %             if length(lf2) ~= 1; error('Something wrong.\n'); end
% % %             ps2 = mesh.t(e2,fn2en(lf2));
% % %             permutation = map(mesh.t(e1,fn2en(j)),mesh.t(i,fn2en(j)));
% % %             permutation
% % %         end
% % %     end
% % % end
...
%     mesh.f(ind,:) = [];
    ...
    [pbnd1,fbnd1,ebnd1] = getface(mesh.f,mesh.p,bndexpr{i,1},bndexpr{i,2});
    
    % get faces on the 2nd boundary 
    [pbnd2,fbnd2,ebnd2] = getface(mesh.f,mesh.p,bndexpr{i,3},bndexpr{i,4});        
    ...
    % modify elcon
    for j=1:nbf
        lf1 = (mesh.t2f(ebnd1(j),:)==fbnd1(j));  % location of face fbnd1(j) on element ebnd1(j)
        lf2 = (mesh.t2f(ebnd2(j),:)==fbnd2(j));  % location of face fbnd2(j) on element ebnd2(j)                
        ...
        fn1 = mesh.f(fbnd1(j),1:end-2);       % corners of face fbnd1(j) 
...
for i = 1:size(bndexpr,1)
    ...
    % get faces on the 1st boundary 
    [pbnd1,fbnd1,ebnd1,vbnd1] = getface(f,mesh.p,bndexpr{i,1},bndexpr{i,2});        
    
    % get faces on the 2nd boundary 
    [pbnd2,fbnd2,ebnd2,vbnd2] = getface(f,mesh.p,bndexpr{i,3},bndexpr{i,4});        
    ...
    for j=1:nbf
        ...
        [~,vp1] = ismember(vbnd1(j,:),mesh.t(ebnd1(j),:)); % location of vbnd1(j,:) on element ebnd1(j)
        [~,vp2] = ismember(vbnd2(j,:),mesh.t(ebnd2(j),:)); % location of vbnd2(j,:) on element ebnd2(j)                        
...
%         end
        ...
        p = mesh.p(mesh.t(ebnd1(j),vp1),:);
        q1  = eval(bndexpr{i,2});        
        p = mesh.p(mesh.t(ebnd2(j),vp2),:);
        q2  = eval(bndexpr{i,4});
        in = xiny(q2, q1);            
        
        % Update tUpdated:
        tm1 = tUpdated(ebnd1(j),vp1);
        tUpdated(ebnd2(j),vp2) = tm1(in);
    ...
    % Decrease entries in tUpdated to account for the vertices removed:            
    verticesToBeRemoved = sort(unique(vbnd2(:)),'descend');    % vertices to be removed, ordered from largest to smallest index.
    verticesToBeRemoved = reshape(verticesToBeRemoved,1,[]);    
    for vv = verticesToBeRemoved
        tUpdated = tUpdated - (tUpdated > vv);
    end
...
for i = 1:size(bndexpr,1)
    ...
    verticesToBeRemoved = reshape(verticesToBeRemoved,1,[]);    
    for vv = verticesToBeRemoved
        if abs(max(max(tUpdated == vv))) ~= 0
            error('A vertex that needed to be removed was not properly removed from t.');
        end
        tUpdated = tUpdated - (tUpdated > vv);
    end
