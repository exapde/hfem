function [elcon,edg,ndof] = mkelconmixed(p,t,f,t2f,elemtype,isEDGface,hybrid,porder,dim)
...
function [philocvl,philocfc,perm,npe,nve,nfe,nde,npfs,face,nvfs] = febasis(porder,dim,elemtype)

elem = unique(elemtype);
nelem = length(elem);
philocvl = cell(nelem,1);
philocfc = cell(nelem,1);
perm = cell(nelem,1);
face = cell(nelem,1);
nvfs = cell(nelem,1);
npe = zeros(nelem,1);
...
npfs = zeros(nelem,6);
for i = 1:nelem
    [philocvl{i},philocfc{i},~,~,perm{i}] = localbasis(porder,dim,elem(i));     
    [npe(i),nve(i)] = size(philocvl{i});
    nfe(i) = length(philocfc{i});    
    for j= 1:nfe(i)
        npfs(i,j) = size(philocfc{i}{j},1);
        nde(i) = nde(i) + size(philocfc{i}{j},1);
    end    
    switch dim
        case 1
            face{i} = [1;2];
...
[philocvls,philocfcs,perms,~,~,~,nde,npfs] = febasis(porder,dim,elemtype);
ncf = dim;
if (dim == 3) && (max(elemtype)>0)  
    ncf = dim+1;
end
ndemax = max(nde(:));
npfmax = max(npfs(:));

hdg = zeros(npfmax*nf,dim);
elcon = zeros(ndemax,ne);
ndof = 0;
for i = 1:nf
    ...
    n = find(elem == elemtype(fe(1)));
    philocvl = philocvls{n};
    philocfc = philocfcs{n}{if1};
    perm = perms{n}{if1};
    npf = size(philocfc,1);
    ...
    hdg(tof,:) = pf;        % hdg nodes on faces
    
    tn = t(fe(1),:);
    dg1 = philocvl(perm,:)*p(tn,:);    
    dg1 = round(dg1/snap)*snap;
    in = xiny(dg1,pf);              % match pf to dg1       
    if if1==1
        aa=0;
    else
        aa = sum(npfs(n,1:(if1-1)));
    end
    ii = ((aa+1):(aa+npf));
    elcon(ii,fe(1)) = tof(in);   % assign dof numbering of face i to elcon from element fe(1)             
    ...
    if fe(2)>0
        ...
        n = find(elem == elemtype(fe(2)));
        philocvl = philocvls{n};        
        perm = perms{n}{if2};
        tn = t(fe(2),:);
        dg2 = philocvl(perm,:)*p(tn,:);        
        dg2 = round(dg2/snap)*snap;
        in = xiny(dg2,pf);              % match pf to dg1          
        if if2==1
            aa=0;
        else
            aa = sum(npfs(n,1:(if2-1)));
        end
        ii = ((aa+1):(aa+npf));        
        elcon(ii,fe(2)) = tof(in);   % assign dof numbering of face i to elcon from element fe(1)                     
    end    
end
