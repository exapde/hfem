...
switch (wcase)
    case 1
        W  = (mu/2)*(trace(CF)+C-3) - mu*log(JF) + (lambda/2)*(JF-1)^2;
    case 2
        W  = (mu/2)*(trace(CF)+C-3) - mu*log(JF) + (lambda/2)*(log(JF))^2;
    case 3
        W  = (mu/2)*(((JF^2)^(-1/3))*(trace(CF)+C)-3) + (lambda/2)*(JF-1)^2;
    case 4
        W  = (mu/2)*(JF^(-2/3)*(trace(CF)+C)-3) + (lambda/2)*(log(JF))^2;
    otherwise
