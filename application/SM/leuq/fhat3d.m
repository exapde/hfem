...
t5 = q13+q31;
t6 = q23+q32;
fh = [tau.*(u1-uh1)+nl1.*(t3+mu.*q11.*2.0)+mu.*nl2.*t4+mu.*nl3.*t5;tau.*(u2-uh2)+nl2.*(t3+mu.*q22.*2.0)+mu.*nl1.*t4+mu.*nl3.*t6;tau.*(u3-uh3)+nl3.*(t3+mu.*q33.*2.0)+mu.*nl1.*t5+mu.*nl2.*t6];
if nargout > 1
    t7 = one.*tau;
    t8 = mu.*nl1.*one;
    t9 = mu.*nl2.*one;
