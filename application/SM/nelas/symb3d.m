...
syms F11 F12 F13 F21 F22 F23 F31 F32 F33 pres mu kappa lambda
...
switch (wcase)
    case 1
        W  = (mu/2)*(trace(CF)-3) - mu*log(JF) + (kappa/2)*(JF-1)^2;
        Wi = (mu/2)*(trace(CF)-3) - mu*log(JF);
        Wv = (kappa/2)*(JF-1)^2;
        Pv = pres*JF*VF;
        g  = JF-1; 
    case 2
        W  = (mu/2)*(trace(CF)-3) - mu*log(JF) + (kappa/2)*(log(JF))^2;
        Wi = (mu/2)*(trace(CF)-3) - mu*log(JF);
        Wv = (kappa/2)*(log(JF))^2;
        Pv = pres*VF;
        g  = log(JF);
    case 3
        W  = (mu/2)*(JF^(-2/3)*(trace(CF))-3) + (kappa/2)*(JF-1)^2;
        Wi = (mu/2)*(JF^(-2/3)*(trace(CF))-3);
        Wv = (kappa/2)*(JF-1)^2;
        Pv = pres*JF*VF;
        g  = JF-1; 
    case 4
        W  = (mu/2)*(JF^(-2/3)*(trace(CF))-3) + (kappa/2)*(log(JF))^2;
        Wi = (mu/2)*(JF^(-2/3)*(trace(CF))-3);
        Wv = (kappa/2)*(log(JF))^2;
        Pv = pres*VF;
        g  = log(JF);
