void flux_leuq2d(double *f, double *f_udg, double *pg, double *udg, double *param, double time, int ng, int nc, int ncu, int nd, int ncd)
{
	...
	for (int i = 0; i <ng; i++) {
		...
		double t4 = u3+u6+2.0;
		double t5 = param2*t4;
		f[0*ng+i] = t5+param1*(u3+1.0)*2.0;
		f[1*ng+i] = t3;
		f[2*ng+i] = t3;
		f[3*ng+i] = t5+param1*(u6+1.0)*2.0;
