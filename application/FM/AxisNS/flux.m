...
% modified for axisymetric
cst = 1;
txx = Re1*c23*(2*ux - vy - cst*v./xr);
txy = Re1*(uy + vx);
tyy = Re1*c23*(2*vy - ux - cst*v./xr);
...
txx_r  =  Re1*c23*((4*ru.*rx-2*rv.*ry)-r.*(2*rux-rvy)).*r1.^3 - cst*Re1*c23*v_r./xr;
txx_ru = -Re1*c23*2*rx.*r1.^2;
txx_rv =  Re1*c23*ry.*r1.^2 - cst*Re1*c23*v_rv./xr;
txx_rE =  zero;
...
tyy_r  =  Re1*c23*((4*rv.*ry-2*ru.*rx)-r.*(2*rvy-rux)).*r1.^3 - cst*Re1*c23*v_r./xr;
tyy_ru =  Re1*c23*rx.*r1.^2;
tyy_rv = -Re1*c23*2*ry.*r1.^2  - cst*Re1*c23*v_rv./xr;
tyy_rE =  zero;
...
f = bsxfun(@times,reshape(xr,[ng 1 1]),f);
f_udg = bsxfun(@times,reshape(xr,[ng 1 1 1]),f_udg);
