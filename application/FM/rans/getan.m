...
if absolute==2 || absolute == 3 % Local Lax-Friedrich    
    [ng,nc] = size(m);    
...
%         rlamm5 = rlam.*rlamm5./(epslm*c) + 0.5*(1 - rlam.*rlam./(epslm*c).^2).*(epslm*cm5);
%     end    
%     
    if absolute==3
        rlam   = un-c;
    ...
    An(:,:,1)  = [rlam, zer, zer, zer, zer];
    Anm(:,:,1,1) = [rlamm1, zer, zer, zer, zer];
    Anm(:,:,1,2) = [rlamm2, zer, zer, zer, zer];
    Anm(:,:,1,3) = [rlamm3, zer, zer, zer, zer];
    Anm(:,:,1,4) = [rlamm4, zer, zer, zer, zer];
    Anm(:,:,1,5) = [rlamm5, zer, zer, zer, zer];

    An(:,:,2)  = [zer, rlam, zer, zer, zer];
    Anm(:,:,2,1)  = [zer, rlamm1, zer, zer, zer];
    Anm(:,:,2,2)  = [zer, rlamm2, zer, zer, zer];
    Anm(:,:,2,3)  = [zer, rlamm3, zer, zer, zer];
    Anm(:,:,2,4)  = [zer, rlamm4, zer, zer, zer];
    Anm(:,:,2,5)  = [zer, rlamm5, zer, zer, zer];

    An(:,:,3)  = [zer, zer, rlam, zer, zer];
    Anm(:,:,3,1)  = [zer, zer, rlamm1, zer, zer];
    Anm(:,:,3,2)  = [zer, zer, rlamm2, zer, zer];
    Anm(:,:,3,3)  = [zer, zer, rlamm3, zer, zer];
    Anm(:,:,3,4)  = [zer, zer, rlamm4, zer, zer];
    Anm(:,:,3,5)  = [zer, zer, rlamm5, zer, zer];

    An(:,:,4)  = [zer, zer, zer, rlam, zer];
    Anm(:,:,4,1)  = [zer, zer, zer, rlamm1, zer];
    Anm(:,:,4,2)  = [zer, zer, zer, rlamm2, zer];
    Anm(:,:,4,3)  = [zer, zer, zer, rlamm3, zer];
    Anm(:,:,4,4)  = [zer, zer, zer, rlamm4, zer];    
    Anm(:,:,4,5)  = [zer, zer, zer, rlamm5, zer];    
    
    An(:,:,5)  = [zer, zer, zer, zer, rlam];
    Anm(:,:,5,1)  = [zer, zer, zer, zer, rlamm1];
    Anm(:,:,5,2)  = [zer, zer, zer, zer, rlamm2];
    Anm(:,:,5,3)  = [zer, zer, zer, zer, rlamm3];
    Anm(:,:,5,4)  = [zer, zer, zer, zer, rlamm4];    
    Anm(:,:,5,5)  = [zer, zer, zer, zer, rlamm5];    
...
An(:,:,1)  = [rlam3+cc1, cc1.*uv+cc2.*nx, cc1.*vv+cc2.*ny, cc1.*h+cc2.*un, zer];
Anm(:,:,1,1) = [rlam3m1+cc1m1, cc1m1.*uv+cc1.*uvm1+cc2m1.*nx, cc1m1.*vv+cc1.*vvm1+cc2m1.*ny, cc1m1.*h+cc1.*hm1+cc2m1.*un+cc2.*unm1, zer];
Anm(:,:,1,2) = [rlam3m2+cc1m2, cc1m2.*uv+cc1.*uvm2+cc2m2.*nx, cc1m2.*vv+cc1.*vvm2+cc2m2.*ny, cc1m2.*h+cc1.*hm2+cc2m2.*un+cc2.*unm2, zer];
Anm(:,:,1,3) = [rlam3m3+cc1m3, cc1m3.*uv+cc1.*uvm3+cc2m3.*nx, cc1m3.*vv+cc1.*vvm3+cc2m3.*ny, cc1m3.*h+cc1.*hm3+cc2m3.*un+cc2.*unm3, zer];
Anm(:,:,1,4) = [rlam3m4+cc1m4, cc1m4.*uv+cc1.*uvm4+cc2m4.*nx, cc1m4.*vv+cc1.*vvm4+cc2m4.*ny, cc1m4.*h+cc1.*hm4+cc2m4.*un+cc2.*unm4, zer];
...
An(:,:,2)  = [cc1, rlam3+cc1.*uv+cc2.*nx, cc1.*vv+cc2.*ny, cc1.*h+cc2.*un, zer];
Anm(:,:,2,1)  = [cc1m1, rlam3m1+cc1m1.*uv+cc1.*uvm1+cc2m1.*nx, cc1m1.*vv+cc1.*vvm1+cc2m1.*ny, cc1m1.*h+cc1.*hm1+cc2m1.*un+cc2.*unm1, zer];
Anm(:,:,2,2)  = [cc1m2, rlam3m2+cc1m2.*uv+cc1.*uvm2+cc2m2.*nx, cc1m2.*vv+cc1.*vvm2+cc2m2.*ny, cc1m2.*h+cc1.*hm2+cc2m2.*un+cc2.*unm2, zer];
Anm(:,:,2,3)  = [cc1m3, rlam3m3+cc1m3.*uv+cc1.*uvm3+cc2m3.*nx, cc1m3.*vv+cc1.*vvm3+cc2m3.*ny, cc1m3.*h+cc1.*hm3+cc2m3.*un+cc2.*unm3, zer];
Anm(:,:,2,4)  = [cc1m4, rlam3m4+cc1m4.*uv+cc1.*uvm4+cc2m4.*nx, cc1m4.*vv+cc1.*vvm4+cc2m4.*ny, cc1m4.*h+cc1.*hm4+cc2m4.*un+cc2.*unm4, zer];
...
An(:,:,3)  = [cc1, cc1.*uv+cc2.*nx, rlam3+cc1.*vv+cc2.*ny, cc1.*h+cc2.*un, zer];
Anm(:,:,3,1)  = [cc1m1, cc1m1.*uv+cc1.*uvm1+cc2m1.*nx, rlam3m1+cc1m1.*vv+cc1.*vvm1+cc2m1.*ny, cc1m1.*h+cc1.*hm1+cc2m1.*un+cc2.*unm1, zer];
Anm(:,:,3,2)  = [cc1m2, cc1m2.*uv+cc1.*uvm2+cc2m2.*nx, rlam3m2+cc1m2.*vv+cc1.*vvm2+cc2m2.*ny, cc1m2.*h+cc1.*hm2+cc2m2.*un+cc2.*unm2, zer];
Anm(:,:,3,3)  = [cc1m3, cc1m3.*uv+cc1.*uvm3+cc2m3.*nx, rlam3m3+cc1m3.*vv+cc1.*vvm3+cc2m3.*ny, cc1m3.*h+cc1.*hm3+cc2m3.*un+cc2.*unm3, zer];
Anm(:,:,3,4)  = [cc1m4, cc1m4.*uv+cc1.*uvm4+cc2m4.*nx, rlam3m4+cc1m4.*vv+cc1.*vvm4+cc2m4.*ny, cc1m4.*h+cc1.*hm4+cc2m4.*un+cc2.*unm4, zer];
...
An(:,:,4)  = [cc1, cc1.*uv+cc2.*nx, cc1.*vv+cc2.*ny, rlam3+cc1.*h+cc2.*un, zer];
Anm(:,:,4,1)  = [cc1m1, cc1m1.*uv+cc1.*uvm1+cc2m1.*nx, cc1m1.*vv+cc1.*vvm1+cc2m1.*ny, rlam3m1+cc1m1.*h+cc1.*hm1+cc2m1.*un+cc2.*unm1, zer];
Anm(:,:,4,2)  = [cc1m2, cc1m2.*uv+cc1.*uvm2+cc2m2.*nx, cc1m2.*vv+cc1.*vvm2+cc2m2.*ny, rlam3m2+cc1m2.*h+cc1.*hm2+cc2m2.*un+cc2.*unm2, zer];
Anm(:,:,4,3)  = [cc1m3, cc1m3.*uv+cc1.*uvm3+cc2m3.*nx, cc1m3.*vv+cc1.*vvm3+cc2m3.*ny, rlam3m3+cc1m3.*h+cc1.*hm3+cc2m3.*un+cc2.*unm3, zer];
Anm(:,:,4,4)  = [cc1m4, cc1m4.*uv+cc1.*uvm4+cc2m4.*nx, cc1m4.*vv+cc1.*vvm4+cc2m4.*ny, rlam3m4+cc1m4.*h+cc1.*hm4+cc2m4.*un+cc2.*unm4, zer];

An(:,:,5)  = [zer, zer, zer, zer, rlam1];
Anm(:,:,5,1)  = [zer, zer, zer, zer, rlam1m1];
Anm(:,:,5,2)  = [zer, zer, zer, zer, rlam1m2];
Anm(:,:,5,3)  = [zer, zer, zer, zer, rlam1m3];
Anm(:,:,5,4)  = [zer, zer, zer, zer, rlam1m4];
