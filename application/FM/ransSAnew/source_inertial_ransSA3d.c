...
void source_inertial_ransSA3d(double *s, double *s_udg, double *pg, double *udg, double *param, double time, int ng, int nc, int ncu, int nd, int ncd, int computeJacobian)
{
// Source term for inertial forces in a rotating frame when solving in relative velocity.
    ...
    if (computeJacobian == 1) {
        for (int i = 0; i <ng; i++) {
            ...
            s_udg[8*ng+i] += W_z*-2.0;
            s_udg[9*ng+i] += t8;
            s_udg[13*ng+i] += W_z*2.0;
            s_udg[15*ng+i] += W_x*-2.0;
            s_udg[19*ng+i] += -t8;
            s_udg[20*ng+i] += W_x*2.0;
        }
    }
}
