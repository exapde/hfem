...
if nd==2                                               
    ...
    %pt  = (pmin/cpar)*log(cpar+exp(cpar*(p/pmin)));
    ptx = px*exp(cpar*(p/pmin))/(cpar+exp(cpar*(p/pmin)));
    rHx = rEx+ptx;                
    pty = py*exp(cpar*(p/pmin))/(cpar+exp(cpar*(p/pmin)));
    rHy = rEy+pty;    
    
    f3   = [Av1*rx, Av1*rux, Av1*rvx,  Av1*rHx, ...
            Av1*ry, Av1*ruy, Av1*rvy,  Av1*rHy];        
    f4   = [Av2*rx, Av2*rux, Av2*rvx,  Av2*rHx, ...
            Av2*ry, Av2*ruy, Av2*rvy,  Av2*rHy];        
    ...
    f5   = [Av3*rx, Av3*rux, Av3*rvx,  Av3*rHx, ...
            Av3*ry, Av3*ruy, Av3*rvy,  Av3*rHy];        
    f6   = [Av4*rx, Av4*rux, Av4*rvx,  Av4*rHx, ...
    ...
    f7   = [Av3*rx, Av3*rux, Av3*rvx,  Av3*rHx, ...
            Av3*ry, Av3*ruy, Av3*rvy,  Av3*rHy];        
    f8   = [Av4*rx, Av4*rux, Av4*rvx,  Av4*rHx, ...
            Av4*ry, Av4*ruy, Av4*rvy,  Av4*rHy];   
else
    r    = udg(1);
    ru   = udg(2);
...
for n=1:10
    ...
    elseif n==5
        f = f5;
        filename1 = ['avflux' num2str(nd) 'd5' '.m'];
    elseif n==6
        f = f6;
        filename1 = ['avflux' num2str(nd) 'd6' '.m'];
    elseif n==7
        f = f7;
        filename1 = ['avflux' num2str(nd) 'd7' '.m'];
    elseif n==8
        f = f8;
        filename1 = ['avflux' num2str(nd) 'd8' '.m'];        
    elseif n==9
        f = s1;
