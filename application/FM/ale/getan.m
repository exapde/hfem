function [An,Anm] = getan(pg,nl,m,param,absolute,time)
%FHAT flux function
...
if absolute==2  % Local Lax-Friedrich    
    [ng,nc] = size(m);    
    ...
    cfy    = 1;
    cfz    = 0;
    rlam   = cfx*abs(un)+cfy*c+cfz*0;
    rlamm1 = cfx*sign(un).*unm1+cfy*cm1;
    rlamm2 = cfx*sign(un).*unm2+cfy*cm2;
    rlamm3 = cfx*sign(un).*unm3+cfy*cm3;
    rlamm4 = cfx*sign(un).*unm4+cfy*cm4;            
...
g1   = 1./gg;
gb1  = 1./gb;
Ginv = [Gg(:,4).*g1 -Gg(:,2).*g1 -Gg(:,3).*g1 Gg(:,1).*g1];

nx   = gb.*(Ginv(:,1).*nl(:,1) + Ginv(:,2).*nl(:,2));              
ny   = gb.*(Ginv(:,3).*nl(:,1) + Ginv(:,4).*nl(:,2));              
nn   = sqrt(nx.^2 + ny.^2);
nx   = nx./nn;
ny   = ny./nn;
...
if absolute
    rlam1   = abs(un+c-Vn);
    rlam1m1 = sign(un+c-Vn).*(unm1+cm1);
    rlam1m2 = sign(un+c-Vn).*(unm2+cm2);
    rlam1m3 = sign(un+c-Vn).*(unm3+cm3);
    rlam1m4 = sign(un+c-Vn).*(unm4+cm4);
...
if absolute
    rlam2   = abs(un-c-Vn);
    rlam2m1 = sign(un-c-Vn).*(unm1-cm1);
    rlam2m2 = sign(un-c-Vn).*(unm2-cm2);
    rlam2m3 = sign(un-c-Vn).*(unm3-cm3);
    rlam2m4 = sign(un-c-Vn).*(unm4-cm4);
...
if absolute
    rlam3   = abs(un-Vn);
    rlam3m1 = sign(un-Vn).*unm1;
    rlam3m2 = sign(un-Vn).*unm2;
    rlam3m3 = sign(un-Vn).*unm3;
    rlam3m4 = sign(un-Vn).*unm4;
...
An  = An.*repmat(nn,[1 nc nc]);
Anm = Anm.*repmat(nn.*gb1,[1 nc nc nc]);
