...
% if max(abs(p(:,1)))<1e-6 %&& min(abs(p(:,2)))<1+2e-3    
%     pinf = 0.011160714285714;
%     ib = 8;
% end
...
switch ib
    ...
    case 3 % Isothermal wall
        ...
        fh(:,1)   = u(:,1)-uh(:,1);        
        fh(:,2:3) = -uh(:,2:3);
        fh(:,4) = -gam*gam1*M2*uh(:,4) + uinf(:,end).*uh(:,1);
        ...
        fh_uh(:,2,2) = -1;
        fh_uh(:,3,3) = -1;
        fh_uh(:,4,:) = [uinf(:,end), zero, zero, -gam*gam1*M2*one];
...
%         fh_udg = cat(3,fh_u,reshape(fh_q,ng,nch,2*nch));        
    ...
    case 7 % supersonic outflow                                              
        ...
        fh_uh(:,3,3) = -1;                
        fh_uh(:,4,4) = -1;         
    case 8 % supersonic outflow and prescribed pressure                                             
        fh = u - uh;                                                
        fh(:,4) = pinf/(gam-1) + 0.5*u(:,2).*u(:,2)./u(:,1) + 0.5*u(:,3).*u(:,3)./u(:,1) - uh(:,4);
        
        fh_udg = zeros(ng,nch,nc);
        fh_udg(:,1,1) = 1; 
        fh_udg(:,2,2) = 1; 
        fh_udg(:,3,3) = 1; 
        fh_udg(:,4,1:nch) = [-0.5*(u(:,2).*u(:,2)+u(:,3).*u(:,3))./(u(:,1).*u(:,1)), u(:,2)./u(:,1), u(:,3)./u(:,1), zeros(ng,1)];
